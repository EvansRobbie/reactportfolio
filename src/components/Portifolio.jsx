...
const Portfolio = () => {
    const portfolioElement = PortfolioData?.map((item) =>{
        const {id, image, website, description,github} = item
        return(
            <div key={id} className=' w-[320px] h-[200px] lg:w-full  relative mx-auto my-6 rounded-xl border-primary  overflow-hidden group shadow-2xl hover:shadow-teal-500'>
            <div className='w-full h-[80%] lg:h-[70%] xl:h-[80%] absolute -bottom-full group-hover:bottom-0 left-0  group-hover:bg-gradient-to-tr from-teal-500 to-[var(--color-bg-primary)]  duration-300 ease-in z-10'/>
  ...
  return (
    <div className='rounded-div drop-shadow-xl lg:max-h-full' id='portfolio'>
